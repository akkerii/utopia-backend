...
import { AgentType, StructuredQuestion, OpenAIModel } from "../types";
...
class OpenAIService {
  ...
  constructor() {
    ...
    this.defaultModel =
      (process.env.OPENAI_MODEL as OpenAIModel) || OpenAIModel.GPT_4O_MINI;
  }
  ...
  // Get default model
  getDefaultModel(): OpenAIModel {
    return this.defaultModel;
  }

  // Validate if model is supported
  isValidModel(model: string): model is OpenAIModel {
  ...
  async generateResponse(
    ...
    try {
      ...
      if (!this.isValidModel(model)) {
        console.warn(
          `Invalid model ${model}, using default ${this.defaultModel}`
        );
  ...
  async generateAgentResponse(
    ...
    temperature: number = 0.7,
    model: OpenAIModel = this.defaultModel,
    generateStructuredQuestions: boolean = false
  ): Promise<{ response: string; structuredQuestions?: StructuredQuestion[] }> {
    try {
      // Validate model
      if (!this.isValidModel(model)) {
        console.warn(
...
REMEMBER: You MUST use the [STRUCTURED_QUESTIONS] format above. Plain questions will not work!`;
      ...
      if (generateStructuredQuestions) {
        ...
        const { response, structuredQuestions } =
          this.parseStructuredQuestions(fullResponse);
  ...
  private parseStructuredQuestions(fullResponse: string): {
    response: string;
    ...
    questionLines.forEach((line) => {
      ...
      if (questionMatch) {
        ...
        let type: "text" | "textarea" | "select" | "multiselect" | "buttons" =
          "text";
